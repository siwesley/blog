...
class Links_Plugin implements Typecho_Plugin_Interface
{
	...
	public static function output_str($pattern=NULL, $links_num=0, $sort=NULL)
	{
		...
		if (!isset($pattern) || $pattern == "" || $pattern == NULL || $pattern == "SHOW_TEXT") {
			$pattern = "<a href=\"{url}\" title=\"{title}\" target=\"_blank\" class=\"item\">{name}</a>\n";
		} else if ($pattern == "SHOW_IMG") {
			$pattern = "<a href=\"{url}\" title=\"{title}\" target=\"_blank\" class=\"item\"><img src=\"{image}\" alt=\"{name}\" /></a>\n";
		} else if ($pattern == "SHOW_MIX") {
			$pattern = "<a href=\"{url}\" title=\"{title}\" target=\"_blank\" class=\"item\"><img src=\"{image}\" alt=\"{name}\" /><span>{name}</span></a>\n";
		}
		$db = Typecho_Db::get();
		$prefix = $db->getPrefix();
		$options = Typecho_Widget::widget('Widget_Options');
